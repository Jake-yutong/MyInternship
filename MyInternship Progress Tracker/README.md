...
**网页版：https://jake-yutong.github.io/MyInternship/**

直接在浏览器打开即可，无需安装任何软件或启动服务。

## 网页版使用说明

1. **打开** https://jake-yutong.github.io/MyInternship/，首次访问会自动加载几条示例记录
2. **新增投递**：点击右上角「+ 新增」，填写公司、岗位、状态等信息后保存
3. **快速填写**：在「职位描述」栏粘贴 JD 文本或招聘链接，系统会自动提取公司名和岗位名
4. **编辑/删除**：点击卡片右侧菜单，或直接点开卡片进行编辑
5. **切换视图**：左侧导航可在卡片视图和甘特图视图之间切换，甘特图会根据各阶段日期自动生成时间轴
6. **数据持久化**：所有数据保存在当前浏览器的 localStorage，关闭页面后仍然存在，下次打开即可继续使用

> 网页版数据仅存储在当前浏览器中。**不要清除浏览器数据**，否则记录会丢失。换浏览器或换设备后看不到同一份数据属于正常现象。
...
- 本地 127.0.0.1 链接需要先在本机启动服务后才能访问（`npm run start:local` 或 `npm run start:server`）
