...
👉 **https://jake-yutong.github.io/MyInternship/**

- 直接在浏览器打开，无需安装任何软件
- 数据保存在浏览器 localStorage，关闭页面后再次打开仍然存在
- 支持作为 PWA 安装到桌面（Chrome/Edge 地址栏右侧出现安装图标）
- 无需登录，无需服务器，纯本地运行

> **注意**：网页版数据存储在当前浏览器中。换浏览器、换设备、或清除浏览器数据后记录不会自动同步。如需跨设备使用，建议使用本地启动方案（数据写入 SQLite 文件，可手动备份）。

**Linux 桌面版（AppImage，类似安装软件）：**

👉 **https://github.com/Jake-yutong/MyInternship/releases/latest/download/MyInternship.AppImage**

- 下载后直接双击运行，无需安装
- 内置本地后端，数据写入 `~/.config/MyInternship/data/applications.sqlite`
## 当前能力
...
| 类型 | 链接 |
|------|------|
| 网页版（直接可用） | https://jake-yutong.github.io/MyInternship/ |
| 仓库地址 | https://github.com/Jake-yutong/MyInternship |
| Linux 桌面版下载 | https://github.com/Jake-yutong/MyInternship/releases/latest/download/MyInternship.AppImage |
| 最新桌面版发布页 | https://github.com/Jake-yutong/MyInternship/releases/latest |
| 本地前端开发地址 | http://127.0.0.1:5173 |
| 本地单服务地址 | http://127.0.0.1:8787 |
| 本地健康检查 | http://127.0.0.1:8787/api/health |

说明：

- 网页版已通过 GitHub Pages 上线，直接点链接即可使用，无需本地服务
- 本地 127.0.0.1 链接需要先在本机启动服务后才能访问
- 完成云端部署后，只需把 http://127.0.0.1:8787 替换成正式域名即可
...
如果你只是想给别人一个"点开就能用"的入口：

- 网页版直接访问：https://jake-yutong.github.io/MyInternship/
- 下载 Linux 桌面版：https://github.com/Jake-yutong/MyInternship/releases/latest/download/MyInternship.AppImage
